...
if __name__ == "__main__":
    ...
    csv_file = open('../dataset_under_window_size_8_xor_bit_op/replay_attack_strength_change_0_8.csv')  # 打开文件
    ...
    for i in range(compare_method_num):
        if(enable[i]>0):
            plt.plot(injection_strength, y_recall[i] * 100, linestyle=line_style[i], color=plot_color[i], label=plot_label[i])

    plt.xlabel("Attack Strength")
    plt.xticks(injection_strength, )
    plt.ylim(-5.0, 105.0)
    ...
    plt.legend(loc="best", edgecolor="k")  # 图例   # upper center lower, left right
    plt.show()
