...
if __name__ == "__main__":
    ...
    csv_file = open('../dataset_under_window_size_8_xor_bit_op/targeted_injection_attack_strength_change_0_8.csv')  # 打开文件
    ...
    for i in range(compare_method_num):
        if(enable[i]>0):
            #plt.plot(x_replay_num, y_accuracy[i] * 100, linestyle=line_style[i], color=plot_color[i], label=plot_label[i])
            #plt.plot(x_replay_num, y_precision[i] * 100, linestyle=line_style[i], color=plot_color[i], label=plot_label[i])
